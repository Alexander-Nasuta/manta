...
for more examples check out the example gallery in the documentation.
You may also want to check out the video below, that showcases Manta's key features.


```{eval-rst}
.. manim:: QuickstartExample
    ...
    class QuickstartMinimalSlideTemplate(MinimalSlideTemplate, VoiceoverScene):
        ...
        def construct(self):
            ...
            with self.voiceover(text=voiceover_text):
                self.play(
                    self.add_logos(),
                    m.Transform(function_text,
                                self.term_text('logo_paths = [<<path to logo>>]\nadd_logos()').move_to(m.ORIGIN)),
                )
